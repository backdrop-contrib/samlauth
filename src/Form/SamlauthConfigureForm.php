...
class SamlauthConfigureForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $idp_options = [];
    foreach ($this->entityTypeManager->getStorage('samlauth_idp')->loadMultiple() as $id => $idp) {
      $idp_options[$id] = $idp->label();
    }
    $form['identity_provider']['default_idp'] = [
      '#type' => 'select',
      '#title' => $this->t('Default Identity Provider'),
      '#description' => $this->t('You can create IdPs at <a href="@url">@url</a>', [
        '@url' => Url::fromRoute('entity.samlauth_idp.collection')->toString(),
      ]),
      '#options' => $idp_options,
      '#default_value' => $config->get('default_idp'),
    ];
    $form['user_info'] = [
